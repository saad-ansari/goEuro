...
@interface TrainViewController ()<RequestDelegate,LGRefreshViewDelegate>
{
...
- (void)viewDidLoad
{
    ...
    if ([SWDefaults trainArray].count > 0)
    {
        [self populateOfferData:[SWDefaults trainArray]];
    }
...
-(void)callOfferData
{
    if([self isConnectionAvailable])
    {
        ...
        [httpManager callGetService:self withName:@"3zmcy"];
    }
    else
    {
...
-(void)populateOfferData:(NSMutableArray *)data
{
    ...
    if (data.count > 0)
    {
        ...
        for (int i = 0; i < data.count; i++)
        {
            ...
            obj.price_in_euros = [NSString stringWithFormat:@"%@",data[i][@"price_in_euros"]];
